...
import { Client, ClientsSelect, Media, Recipe } from '@/payload-types'
import {
  Edit as EditIcon,
        ...
        props.docs.map((doc) => (
          ...
          >
            <ListItemAvatar>
              <Avatar
                variant="square"
                src={(doc.image as Media)?.thumbnailURL || undefined}
      ...
      {props.loading ? (
        ...
        props.docs.map((doc) => (
          <Grid key={doc.id} size={{ xs: 12, sm: 6, md: 4, lg: 3 }}>
            <Card>
              <CardMedia
                component={(doc.image as Media)?.firebaseURL ? 'img' : ('div' as 'img' | 'div')}
                image={
                  (doc.image as Media)?.firebaseURL
                    ? String((doc.image as Media)?.firebaseURL)
                    : undefined
                }
                alt={doc.title ?? undefined}
                sx={{
              ...
              <CardContent>
                ...
                {doc.clients && doc.clients.length > 0 && (
                  <Clients>
                    {doc.clients
                      .filter(
                        (client): client is { id?: string | null; client: Client } =>
                          typeof client?.client === 'object' && client.client !== null,
                      )
                      .map((client) => (
                        <Avatar
                          key={client.id}
                          src={(client.client.image as Media)?.thumbnailURL ?? undefined}
